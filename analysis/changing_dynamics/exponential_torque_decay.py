...
TORQUE_DECAYS = [0.0, 0.05, 0.1, 0.2, 0.5]
...
filters = {
    ...
    "config.pendulum_torque_decay_rate": {"$in": TORQUE_DECAYS},
...
if cache_file.exists():
    ...
    env_dfs = {}
    for decay in TORQUE_DECAYS:
        run_for_decay: Run = next(
            r for r in runs if r.config.get("pendulum_torque_decay_rate") == decay
        )
        ...
        env_dfs[decay] = env_df.dropna(subset=["env_params/max_torque"])
    ...
    df["decay"] = df["run_id"].map(
        {r.id: r.config.get("pendulum_torque_decay_rate") for r in runs}
    )
...
fig, axes = plt.subplots(
    len(row_vals) + 1,
    len(TORQUE_DECAYS),
    figsize=(4 * len(TORQUE_DECAYS), 2.5 * (len(row_vals) + 1)),
    sharex=True,
)

# ---- Top row: env parameter per decay
for j, decay in enumerate(TORQUE_DECAYS):
    ax = axes[0, j]
    ...
    ax.plot(x_env, env_df["env_params/max_torque"], color="black")
    ax.set_title(f"decay={decay}")
    ax.grid(True, alpha=0.3)

axes[0, 0].set_ylabel("max_torque")
